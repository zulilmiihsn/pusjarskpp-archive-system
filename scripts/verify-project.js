...
const appController = read('WorkspaceController.js') + '\n' + read('DriveController.js') + '\n' + read('TemplateController.js') + '\n' + read('AccountController.js') + '\n' + read('SecurityHelpers.js') + '\n' + read('ArchiveController.js') + '\n' + read('SubActivityController.js') + '\n' + read('SettingsController.js') + '\n' + read('PureFunctions.js');
...
const authService = read('AuthService.js');
['ClientState.html', 'ClientApi.html', 'ClientRouter.html', 'ClientRouteState.html', 'ClientNavigation.html', 'ClientLogin.html', 'ClientSettings.html'].forEach(function (file) {
  const source = read(file).replace(/^\s*<script>\s*/, '').replace(/\s*<\/script>\s*$/, '');
  new Function(source);
});
const configConstants = read('ConfigConstants.js');
const configHelpers = read('ConfigHelpers.js');
const configRepo = read('ConfigRepository.js');
const configService = read('ConfigService.js');
const config = configConstants + '\n' + configHelpers + '\n' + configRepo + '\n' + configService;
const driveService = read('DriveService.js') + '\n' + read('DriveHelpers.js') + '\n' + read('DriveArchiveService.js');
const readme = read('README.md');
const spreadsheetService = read('SpreadsheetService.js') + '\n' + read('SheetHelpers.js');
...
assert(
  ...
  'Both archive branches must receive restricted access.'
);
assert(
  !/wsEnsureRestrictedSharing_\(systemFolder, report\)/.test(workspaceSetup),
  'System/config folder must not be force restricted.'
);
assert(
...
const initWorkspaceBody = read('WorkspaceController.js').match(/initializeWorkspace: function \(payload\) \{([\s\S]*?)\n {2}\},/);
assert(
...
assert(read('SecurityHelpers.js').includes('function requireAdminIfWorkspaceSecured_') && read('SecurityHelpers.js').includes('hasActiveAdminAccount_'), 'Workspace auth guard must allow first-run or half-initialized recovery before an admin exists.');
assert(read('SettingsWorkspaceService.js').includes('const adminResult = AuthService.saveDefaultAdmin();'), 'Workspace initialization must create the default admin through the internal auth service.');
assert(!read('SettingsWorkspaceService.js').includes('AccountController.saveDefaultAdmin()'), 'Workspace initialization must not call the admin-protected saveDefaultAdmin endpoint.');
...
assert(read('Code.js').includes('function adoptExistingArchives'), 'Existing archive adoption must be exposed to Apps Script clients.');
assert(read('Code.js').includes('function previewExistingArchives'), 'Existing archive adoption must support a preview endpoint.');
...
assert(read('SettingsTemplateService.js').includes('normalizeHexColor_(meta.color'), 'Template data must merge stored category colors.');
assert(read('SettingsTemplateService.js').includes('ConfigRepository.setTemplateCategory'), 'Template category assignment must persist to config.');
assert(read('SpreadsheetService.js').includes('updateRekapSubActivityIdentity'), 'Sub-activity mapping changes must update the rekap identity row.');
assert(read('SpreadsheetService.js').includes('reconcileGlobalArchiveNumbers'), 'Sync Drive must reconcile one global archive sequence across the year.');
assert(read('SpreadsheetService.js').includes('const existingRow = findRekapRowForSubActivity_'), 'Rekap row creation must be idempotent.');
assert(read('SpreadsheetService.js').includes('findPossibleRekapRowsFromLookup_') && read('SpreadsheetService.js').includes('SpreadsheetService.updateRekapSummary(activity, sub, {})'), 'Sync Drive must safely recreate truly missing rekap rows and calculate them from detail sheets.');
assert(read('SheetHelpers.js').includes('PORTAL_ARSIP_ROW_ID|'), 'Rekap rows must carry a stable sub-activity identity marker.');
assert(read('SheetHelpers.js').includes('findRekapRowForSubActivityFromLookup_'), 'Rekap lookup must validate stale row hints against stable identity.');
assert(!read('SheetHelpers.js').includes('uraian.indexOf(targetName)') &&
  !read('SheetHelpers.js').includes('targetName.indexOf(uraian)'),
  'Rekap identity must never use fuzzy substring matching for numbered or Roman-numeral sub-activities.');
assert(!read('SheetHelpers.js').includes('getDocumentProperties()') &&
  !read('SheetHelpers.js').includes("setFormula('=1/2')"),
  'Formula separator detection must work in standalone web apps without Document Properties or cell probes.');
assert(read('PureFunctions.js').includes('buildGlobalArchiveNumberPlan_') &&
  read('SpreadsheetService.js').includes('planGlobalArchiveNumbers') &&
  read('SheetHelpers.js').includes('writeDetailArchiveNumber_'),
  'Archive numbering must use one global plan and mirror it to rekap/detail sheets.');
assert(!read('PureFunctions.js').includes('normalizeRekapArchiveSequence_') &&
  !read('SheetHelpers.js').includes('repairRekapArchiveSequence_'),
  'Per-spreadsheet 1..N normalization must not return.');
assert(!read('SpreadsheetService.js').includes('cascadeNomorBerkasShift'),
  'Legacy per-activity numbering cascade must not return.');
assert(!read('SheetHelpers.js').includes('locks.nomorBerkas !== false && expectedNumber'), 'Canonical Nomor Berkas repair must not be bypassed by manual metadata locks.');
assert(read('MetadataService.js').includes('normalized.no_berkas = normalized.no_berkas || resolveSubActivityArchiveNumber_'), 'Existing detail Nomor Berkas must not be overwritten by a config fallback.');
assert(read('SpreadsheetService.js').includes('REKAP_SUMMARY_COLUMNS.nomorBerkas') &&
  read('SpreadsheetService.js').includes('REKAP_SUMMARY_COLUMNS.noFolder'),
  'Global archive number must be mirrored to both Rekap identity columns.');
assert(read('ConfigRepository.js').includes('bulkUpdateSubActivityNumbering') &&
  read('ConfigRepository.js').includes('local_sort_order'),
  'Global and local archive ordering must be persisted together in config.');
assert(read('SubActivityController.js').includes('formalArchiveName') && read('SubActivityController.js').includes('rekapRowNumber'), 'Sub-activity rename must accept mapping metadata, not only the folder name.');
...
assert(read('ArchiveController.js').includes('buildArchiveFileIndex_') && read('ArchiveController.js').includes('cur.depth + 1'), 'Existing archive adoption must search nested target folders (file index built once per sub-activity).');
assert(read('SubActivityController.js').includes('persistRekapRowNumber_'), 'New/synced sub-activities must persist their rekap row number.');
assert(read('SubActivityController.js').includes('markRekapSubActivityInactive'), 'Sub-activity deactivation must mark the rekap row.');
assert(read('SpreadsheetService.js').includes('clearRekapSubActivityInactiveMark'), 'Sub-activity restore must clear stale inactive rekap notes.');
...
assert(read('MetadataService.js').includes('normalized.jumlah') && read('MetadataService.js').includes('? metadata.jumlah : 1'), 'Archive metadata normalization must default Jumlah to one document per detail row.');
...
assert(read('Code.js').includes('function updateSubActivityMetadata'), 'updateSubActivityMetadata endpoint must be exposed to Apps Script clients.');
assert(read('SpreadsheetService.js').includes('updateRekapDocumentMetadata'), 'Sub-activity metadata editor must be able to update the rekap sheet.');
...
assert(workspaceSetup.includes("['nomor_surat', 'Nomor Surat'") && read('ClientState.html').includes("nomor_surat: 'Nomor Surat'"), 'Nomor Surat must be available in metadata config and labels.');
...
assert(workspaceSetup.includes('wsBuildLeafSubActivityEntries_'), 'Workspace setup must map leaf folders as final sub-activities.');
assert(read('SubActivityController.js').includes('wsBuildLeafSubActivityEntries_') && read('SubActivityController.js').includes('entry.groupName || entry.parentFolderName'), 'Sub-activity sync must use leaf folders and preserve leadership group context.');
...
assert(authService.includes("console.info('LOGIN_PERF '") && read('SettingsBootstrapService.js').includes("console.info('BOOTSTRAP_PERF '"), 'Login and bootstrap must emit phase timing diagnostics.');
...
assert(!read('SystemLogger.js').includes('Session.getActiveUser'), 'System logger identity must come from portal session context.');
assert(read('Code.js').includes("return 'ACCESS_DENIED'") && read('Code.js').includes("return 'AUTH_ERROR'"), 'Authorization denial must be distinct from invalid session.');
...
assert(read('SecurityHelpers.js').includes("SystemLogger.warn('ACCESS_DENIED'") && read('SecurityHelpers.js').includes('failedCheck'), 'Access denials must write diagnostic context to system_logs.');
...
assert(read('ArchiveController.js').includes('parseDocumentContent'), 'Auto-parse backend endpoint must exist in ArchiveController.');
assert(read('ArchiveController.js').includes('ParseEngine.analyze'), 'Auto-parse must use ParseEngine for extraction.');
assert(read('Code.js').includes('function parseDocumentContent'), 'Auto-parse must be exposed as a public Apps Script endpoint.');
assert(read('ParseEngine.js').includes('analyze'), 'ParseEngine module must exist with analyze entry point.');
assert(read('ParseEngine.js').includes('analyzeStructure'), 'ParseEngine must include document structure analysis.');
assert(read('ParseEngine.js').includes('classifyDocumentType'), 'ParseEngine must include document type classification.');
...
assert(read('MetadataService.js').includes('extractKlasifikasiAkses_'), 'Klasifikasi akses extractor must exist in MetadataService.');

// Regresi audit holistik: AppController.js dihapus saat refactor split — tak boleh ada
// referensi tersisa (akan ReferenceError di runtime). Gunakan WorkspaceController/DriveService.
assert(!read('Code.js').includes('AppController.') && !read('SubActivityController.js').includes('AppController.'),
...
assert(read('ConfigRepository.js').includes('getActivities: function') &&
  read('ConfigRepository.js').includes('getSubActivities: function') &&
  read('ConfigRepository.js').includes('getSubActivityById: function'),
...
assert(read('ConfigConstants.js').includes("DOCUMENT_TYPES: 'config_document_types'"),
...
const redundantRootMirrors = fs.readdirSync(root).filter(file =>
  file.endsWith('.gs') && fs.existsSync(path.join(root, file.replace(/\.gs$/, '.js')))
);
assert(redundantRootMirrors.length === 0,
  'Root-level .gs mirrors of .js files are redundant: ' + redundantRootMirrors.join(', '));
assert(!fs.existsSync(path.join(root, '_DebugAutofill.js')),
